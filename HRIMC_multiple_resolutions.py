...
def RLD_HRIMC_circle(input_dir, output_dir, x0,  resolution, iterations = 7):
    ...
    for filename in os.listdir(input_dir):
        if filename.endswith(".tiff") or filename.endswith(".tif"):
            ...
            for layer in range(scan_data.shape[0]):
                ...
                if resolution == 333: 
                    #Here, we model overlapping acquisitions and annotate regions with values according to laser pass map
                    Passes = np.array([7,6,5,8,7,
                                       ...
                                       2,1,1,4,3,
                                       2,1])
                    #Here, we summarise the proportional area of each of these passes
                    Contributions = np.array([0.0196,0.00107,0.00107,0.0034,0.0196,
                    ...
                    Passes = np.array([4, 4,3,2,2,2, 2, 4,3,2,2,2, 4,3,2,2,2,2,1,2,1,2,1,1,
                                       2,1,1,1,1, 2, 2,1,1,1,1, 1])

                    Contributions = np.array([0.02575, 0.02575,0.0406,0.0406,0.01435,0.02575,
            ...
            #imwrite(output_path, scan_data) Can choose to add in a normalized image for quick comparison but not necessary
...
# %%
#HR-IMC (333 nm) 
RLD_HRIMC_circle(input_dir, output_dir, x0 = 8, resolution = 333) #resolution = 500 for 500nm HR-IMC. Adjust x0 as described. 
