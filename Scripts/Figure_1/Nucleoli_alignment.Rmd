---
title: "Nucleoli alignment and Analysis"
...
```{r} 
#Here we run the function to quantify the correlations across the three nucleoli regions
...
rownames(data) <- c("IMC", "Oversampled", "PSF-transformed")
data <- melt(data)
colnames(data) <- c("Method", "Nucleolus", "Correlation")
```


```{r Plotting the correlations}
mean_values <- data %>% group_by(Method) %>% summarize(mean_value = mean(Correlation))

ggplot(data, aes(x = Method, y = Correlation, fill = Method, color = Method)) +
  geom_boxplot() +
  geom_jitter(width = 0.1, alpha = 0.5, size = 3, stroke = 0.5, aes(color = Method)) +
  geom_label_repel(data = mean_values, aes(x = Method, y = mean_value, label = sprintf("hat(mu)== %.2f", mean_value)), 
  ...
  scale_fill_manual(values = c(alpha("#204a77",0.5), alpha("#A90C38",0.6), alpha("#F5B35E", 0.6))) + 
  scale_color_manual(values = c("#204a77", "#A90C38", "#F5B35E")) +
  theme_minimal() +
  theme(panel.border = element_rect(color = "black", fill = NA, size = 1),
        axis.text.x = element_text(angle = 25, hjust = 1)) +
  stat_compare_means(
    comparisons = list(c("IMC", "Oversampled"), c("IMC", "PSF-transformed"), c("Oversampled", "PSF-transformed")),
    method = "t.test", paired = FALSE, label = "p.format", size = 3)
